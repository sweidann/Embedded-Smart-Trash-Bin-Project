...
int main() {
    ...
    xTaskCreate(task_flame_detection, "Flame Detection", 512, NULL, 1, NULL);
...
void task_flame_detection(void *param) {
    while (1) {
        ...
        if (flame_detected) {
            buzzer_play_tone(BUZZER_PIN, 1000, 0);  // Continuous tone
...
void task_distance_monitoring(void *param) {
    while (1) {
        ...
        if (distance < 2.0f) {  // Threshold for "full" bin
